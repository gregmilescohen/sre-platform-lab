...
import pytest
from app.telemetry import _JsonFormatter, _configure_json_logging, setup_tracing
...
class TestJsonFormatter:
    ...
    def test_newlines_in_message_are_json_escaped(self) -> None:
        ...
        output = self.formatter.format(record)
        # The raw output must be a single JSON line (newline is escaped inside the value)
        assert output.count("\n") == 0
        parsed = json.loads(output)
        assert "\n" in parsed["message"]
